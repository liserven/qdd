...
class Record  extends Controller
{
    ...
    public function recordList(){
        ...
        $sumData=Db::name('accountrecord')->where($where)->field('FlowType,sum(Amount) as Amount')->group('FlowType')->select();
        $count=Db::name('accountrecord')->where($where)->order('ID desc')->count();//获取满足条件的总记录数
        $list=Db::name('accountrecord')->where($where)->order('ID desc')->paginate();//根据条件分页输出
