...
class PlacesAccount  extends Controller
{
    ...
    public function recordList(){
        ...
        $sumData=Db::name('accountplaces')->where($where)->field('FlowType,sum(Amount) as Amount')->group('FlowType')->select();
        $count=Db::name('accountplaces')->where($where)->order('ID desc')->count();//获取满足条件的总记录数
        $list=Db::name('accountplaces')->where($where)->order('ID desc')->paginate();//根据条件分页输出

        $staticPageArr=$staticArr=[['FlowType'=>'充值','Amount'=>0],['FlowType'=>'扣币','Amount'=>0],['FlowType'=>'转入','Amount'=>0],['FlowType'=>'转出','Amount'=>0],['FlowType'=>'返币','Amount'=>0]];
        if(!empty($sumData)){
        ...
        foreach ($list as $k=>$v){
            ...
            if($v['FlowType']=='返币'){
                $staticPageArr[4]['Amount']+=$v['Amount'];
            }
        }
