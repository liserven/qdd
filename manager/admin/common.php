...
function arrayIndexConvertLower(&$list){
    foreach ($list as $k=>$v){
...
function getusertypebyid($tid){ //得到用户类型通过类型id
    $usertype=array_change_key_case(Db::name('usertype')->where("id=".$tid)->find());
    return $usertype["name"];
}

function getcatenamebycid($cid){ //根据新闻栏目ID获得栏目名称
    $cateT=Db::name('articlecategory')->where("id=".$cid)->find();
    return $cateT["categoryname"];
}

function getcolumncatenamebycid($cid){ //根据栏目ID获得栏目名称
    $cateT= Db::name('columncategory')->where("id=".$cid)->find();
    return $cateT["categoryname"];
}
function getsuppliername($supplierid){ //根据商家id获取商家名称
    $cateT= Db::name('supplier')->field('Name')->where("id=".$supplierid)->find();
    return $cateT["Name"];
}
function getpname($UserId){ //根据userid获取推荐人姓名
    $where['UserId'] = $UserId;
    $userinfo= Db::name('usermsg')->field('TrueName')->where($where)->find();
